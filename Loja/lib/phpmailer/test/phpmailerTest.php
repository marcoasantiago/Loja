...
EOT;
    ...
    public function testMiscellaneous()
    {
        ...
        //Test pathinfo
        $a = '/mnt/files/é£åæ¨ ååè«.mp3';
        $q = PHPMailer::mb_pathinfo($a);
        $this->assertEquals($q['dirname'], '/mnt/files', 'UNIX dirname not matched');
        $this->assertEquals($q['basename'], 'é£åæ¨ ååè«.mp3', 'UNIX basename not matched');
        $this->assertEquals($q['extension'], 'mp3', 'UNIX extension not matched');
        $this->assertEquals($q['filename'], 'é£åæ¨ ååè«', 'UNIX filename not matched');
        $this->assertEquals(
            PHPMailer::mb_pathinfo($a, PATHINFO_DIRNAME),
            '/mnt/files',
            'Dirname path element not matched'
        );
        $this->assertEquals(PHPMailer::mb_pathinfo($a, 'filename'), 'é£åæ¨ ååè«', 'Filename path element not matched');
        $a = 'c:\mnt\files\é£åæ¨ ååè«.mp3';
        $q = PHPMailer::mb_pathinfo($a);
        $this->assertEquals($q['dirname'], 'c:\mnt\files', 'Windows dirname not matched');
        $this->assertEquals($q['basename'], 'é£åæ¨ ååè«.mp3', 'Windows basename not matched');
        $this->assertEquals($q['extension'], 'mp3', 'Windows extension not matched');
        $this->assertEquals($q['filename'], 'é£åæ¨ ååè«', 'Windows filename not matched');
    }
}
